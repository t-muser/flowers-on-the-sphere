"""DataModule for the preprocessed (already-regridded) 3-D global-ocean dataset.

Reads from the lat/lon zarr tree produced by
``scripts/preprocess_global_ocean_3d_to_latlon.py``. Compared to
:class:`fots.data.global_ocean_3d.GlobalOcean3DDataModule` the per-step
``__getitem__`` is reduced to a strided zarr read + level subset +
mask × normalize + concat (depth, doy) — no IDW regrid, no uv rotation,
no land imputation in the hot path.

On-disk schema (per run.zarr, written by the preprocessor):

* ``theta``, ``salt``, ``u``, ``v``: ``(time, level, lat, lon)`` float32
* ``eta``: ``(time, lat, lon)`` float32

Channel layout matches :func:`datagen.mitgcm.global_ocean.regrid.field_names_3d`,
so ``stats.json`` keys (``theta_k01`` … ``eta``) and ``valid_mask`` from
``field_masks_3d_ll`` are reused unchanged.
"""
...
def _peek_levels(any_run_zarr: Path) -> np.ndarray:
    ds = xr.open_zarr(str(any_run_zarr), consolidated=True)
    return ds["level"].values.astype(np.int64)


class _GlobalOcean3DLatLonWindowDataset(Dataset):
    """Sliding-window sampler over already-regridded run.zarrs."""
    ...
    def _window(self, ds: xr.Dataset, t0: int, T: int) -> tuple[np.ndarray, np.ndarray]:
        window = ds.isel(time=slice(t0, t0 + T))
        sub = window.sel(level=list(int(v) for v in self._level_idx))
        # On-disk channel layout: [theta_kN..., salt_kN..., u_kN..., v_kN..., eta]
        theta = sub["theta"].values
        salt  = sub["salt"].values
        u     = sub["u"].values
        v     = sub["v"].values
        eta   = window["eta"].values  # (T, H, W)
        latlon = np.concatenate(
            [theta, salt, u, v, eta[:, None, :, :]], axis=1,
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        x_dyn, x_time = self._window(ds, t0, self.n_steps_input)
        y_dyn, _ = self._window(
...
class GlobalOcean3DLatLonDataModule(AbstractDataModule):
    """Already-regridded 3-D global-ocean DataModule.

    Drop-in replacement for ``GlobalOcean3DDataModule`` once the
    preprocessor has run; reads the same level subsets, applies the same
    masks/stats, but skips the per-batch IDW + uv rotation.
    """
    ...
    def __init__(
        ...
        max_rollout_steps: Optional[int] = None,
        num_workers: int = 0,
        dataset_name: str = "global_ocean_3d_latlon",
        rank: int = 0,
        world_size: int = 1,
        ...
        # Channel count matches the pre-regridded layout: 4 dynamic 3-D fields
        # × Nlevel + 1 (eta) = 4*Nlevel + 1 dynamic channels.
        n_dynamic = 4 * Nlevel + 1
        ...
        if not grid_zarr.exists():
            raise FileNotFoundError(
                f"{grid_zarr} not found — copy the source-side grid.zarr "
                "into the preprocessed tree (the preprocessor does this)."
            )
        # Build grid_ll for masks + depth_ll. We don't need the regrid
        # weights at training time, but build() also fills them — the cost
        # is ~2 s once at startup.
        ...
        if self._grid_ll.mask_c_3d_ll is None or self._grid_ll.mask_w_3d_ll is None:
            raise RuntimeError(
                f"{grid_zarr} has no per-level masks; regenerate it with "
                "the 3-D extract_grid before training."
            )
        logger.info(
            "%s pre-regridded; (%d, %d) grid  Nlevel=%d  C_dyn=%d",
            dataset_name, nlat, nlon, Nlevel, n_dynamic,
        )
        ...
        def _make(split: str, full_trajectory: bool = False) -> _GlobalOcean3DLatLonWindowDataset:
            return _GlobalOcean3DLatLonWindowDataset(

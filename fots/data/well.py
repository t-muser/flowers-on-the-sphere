...
class WellDataset(Dataset):
    ...
    Args:
        ...
        stats:
            Optional dict containing normalization statistics. If provided, will be used
            instead of loading from normalization_path. Should match format expected by
            normalization_type (e.g., ZScoreNormalization).
        n_steps_input:
    ...
    def __init__(
        ...
        # Parse meta_scalars config
        self.meta_scalars = meta_scalars or []
        # Build lookup: scalar_name -> normalization_type ("log" or "linear")
        self.meta_scalar_names = [cfg["name"] for cfg in self.meta_scalars]
        self.meta_scalar_norms = {cfg["name"]: cfg["normalization"] for cfg in self.meta_scalars}
        ...
        if use_normalization and normalization_type:
            # If stats provided directly from config, use them; otherwise load from file
            if stats is not None:
                try:
                    self.norm = normalization_type(
                        ...
                        log_transform_scalars=self.meta_scalar_norms,
                    )
                except Exception as e:
                    warnings.warn(
                        f"Error creating normalization from provided stats: {e}. Proceeding without normalization.",
                        UserWarning,
                    )
                    self.norm = None
            else:
    ...
    def _build_metadata(self):
        ...
        names = set()
        ndims = set()
        bc_types_list = None  # Will store ordered BC types from first file
        ...
        for index, file in enumerate(self.files_paths):
            with (
                ...
                if bc_types_list is None:
                    bc_types_list = current_bc_types
                else:
                    assert bc_types_list == current_bc_types, (
                        f"Inconsistent boundary conditions: {bc_types_list} vs {current_bc_types} in {file}"
                    )
        ...
        # BCs
        self.bc_types = bc_types_list  # List of boundary condition types in spatial dim order
        self.num_bcs = len(self.bc_types)  # Number of boundary condition types included in data
    ...
    def _reconstruct_scalars(self, file, cache, sample_idx, time_idx, n_steps, dt):
        ...
        for scalar_name in file["scalars"].attrs["field_names"]:
            ...
            if scalar_name in cache:
                ...
                # Apply log transform if specified (before normalization)
                if scalar_name in self.meta_scalar_norms and self.meta_scalar_norms[scalar_name] == "log":
                    scalar_data = torch.log(scalar_data)

                # Normalize
                if self.use_normalization and self.norm:
                    scalar_data = self.norm.normalize(scalar_data, scalar_name)
                # If constant, try to cache
                if (
    ...
    def _postprocess_data(
        ...
        # Note: constant_scalars will be kept as a separate meta tensor (B, N)
        # Variable scalars: concatenate all
        data["variable_scalars"] = [scalar.unsqueeze(-1) for _, scalar in data["variable_scalars"].items()]
        if data["variable_scalars"]:
            data["variable_scalars"] = torch.concatenate(data["variable_scalars"], dim=-1)
        else:
            data["variable_scalars"] = torch.tensor([])

        # Constant scalars (meta): only include those specified in meta_scalars config, in order
        if len(self.meta_scalar_names) > 0:
            # Filter and reorder according to config
            data["constant_scalars"] = [
                data["constant_scalars"][name].unsqueeze(-1)
                for name in self.meta_scalar_names
                if name in data["constant_scalars"]
            ]
            if data["constant_scalars"]:
                data["constant_scalars"] = torch.concatenate(data["constant_scalars"], dim=-1)
            else:
                data["constant_scalars"] = torch.tensor([])
        else:
            # If no meta_scalars specified, empty tensor (no meta conditioning)
            data["constant_scalars"] = torch.tensor([])
    ...
    def _construct_sample(
        ...
        # Input/Output split
        # Permute fields to channels-first: the Well stores tensors as
        # (..., *spatial, C); downstream models (Zinnia, SFNO, FNO, ...) and
        # `Trainer._fold_time_into_channels` all assume (..., C, *spatial).
        variable_fields = data["variable_fields"].movedim(-1, 1)  # T x C x *spatial
        constant_fields = data["constant_fields"]
        if constant_fields.numel() > 0:
            constant_fields = constant_fields.movedim(-1, 0)  # C x *spatial

        sample = {
            "input_fields": variable_fields[: self.n_steps_input],   # Ti x C x *spatial
            "output_fields": variable_fields[self.n_steps_input :],  # To x C x *spatial
            "constant_fields": constant_fields,                      # C x *spatial
            ...
            "meta": data["constant_scalars"],  # N (metadata/constant scalars)
        }
        ...
        if self.return_grid:
            sample["space_grid"] = data["space_grid"]  # *spatial x D
...
class LeadTimeDataset(WellDataset):
    """Dataset for lead-time prediction training.

    Instead of predicting consecutive timesteps (t+1, t+2, ...), this dataset
    samples a random lead-time k and returns only (input_t, target_t+k) pairs.
    The lead-time k is appended to the meta tensor for FiLM conditioning.

    During training: k is sampled uniformly from [1, max_lead_time]
    During validation: k is fixed (set via fixed_lead_time parameter)

    Args:
        max_lead_time: Maximum lead-time k to sample from (default: 20)
        fixed_lead_time: If set, use this fixed lead-time instead of random sampling
        All other args are passed to WellDataset
    """
    ...
    def __init__(
        ...
        *args,
        ...
        fixed_lead_time: Optional[int] = None,
        **kwargs,
    ):
        # Force dt=1 (no variable stride in lead-time mode)
        kwargs['min_dt_stride'] = 1
        kwargs['max_dt_stride'] = 1

        # Store lead-time params before calling super
        self.max_lead_time = max_lead_time
        self.fixed_lead_time = fixed_lead_time
        # For curriculum learning: current max lead time (can be updated per epoch)
        self.current_max_lead_time = max_lead_time

        # For window calculation, we need n_steps_input + max_lead_time timesteps
        # Set n_steps_output to 1 since we only output one timestep (at t+k)
        kwargs['n_steps_output'] = 1

        # Override max_rollout_steps to max_lead_time for window calculation
        kwargs['max_rollout_steps'] = max_lead_time

        super().__init__(*args, **kwargs)

        # Recalculate windows based on max_lead_time
        self._recalculate_lead_time_windows()

    def _recalculate_lead_time_windows(self):
        """Recalculate window indices based on max_lead_time.

        We need at least n_steps_input + max_lead_time timesteps per window.
        """
        self.file_index_offsets = [0]
        self.n_windows_per_trajectory = []

        for file_idx, n_steps in enumerate(self.n_steps_per_trajectory):
            # Number of valid starting positions for lead-time sampling
            # We need: time_idx + n_steps_input + max_lead_time <= n_steps
            # So: time_idx <= n_steps - n_steps_input - max_lead_time
            windows = n_steps - self.n_steps_input - self.max_lead_time
            windows = max(0, windows)
            self.n_windows_per_trajectory.append(windows)
            self.file_index_offsets.append(
                self.file_index_offsets[-1] + self.n_trajectories_per_file[file_idx] * windows
            )

        # Fix first offset to -1 as in base class
        self.file_index_offsets[0] = -1
        self.len = self.file_index_offsets[-1]

        # Update restriction set if it exists
        if self.restriction_set is not None:
            # Filter out indices that are now out of range
            self.restriction_set = self.restriction_set[self.restriction_set < self.len]
            self.len = len(self.restriction_set)

    def set_current_max_lead_time(self, k: int):
        """Set the current maximum lead time for curriculum learning.

        This allows gradually increasing the prediction horizon during training.
        For example: epoch 1 samples k in [1,1], epoch 2 in [1,2], etc.

        Args:
            k: New maximum lead time (clamped to [1, max_lead_time])
        """
        self.current_max_lead_time = max(1, min(k, self.max_lead_time))

    def _load_one_sample(self, index):
        """Load sample with lead-time k, fetching only timesteps t and t+k."""
        # Find specific file and local index
        if self.restriction_set is not None:
            index = self.restriction_set[index]
        ...
        # Sample or use fixed lead-time
        if self.fixed_lead_time is not None:
            k = self.fixed_lead_time
        else:
            # Uniform sampling from 1 to current_max_lead_time (inclusive)
            # current_max_lead_time may be less than max_lead_time for curriculum learning
            k = np.random.randint(1, self.current_max_lead_time + 1)

        # Store k for use in _construct_sample
        self._current_lead_time = k
        ...
        with h5.File(
            ...
            **IO_PARAMS["h5py_params"],
            ...
            # Fetch n_steps_input frames starting at time_idx, plus 1 frame at time_idx + n_steps_input - 1 + k
            # For simplicity, we'll fetch frames [time_idx : time_idx + n_steps_input + k]
            # and then select the appropriate ones in _construct_sample
            n_steps_total = self.n_steps_input + k
    ...
    def _construct_sample(
        ...
        """Construct sample with lead-time appended to meta.

        Takes input from timesteps [0:n_steps_input] and output from timestep [n_steps_input + k - 1].
        Normalizes lead-time k to [0, 1] using (k-1)/(max_k-1).
        """
        k = self._current_lead_time

        # Input is first n_steps_input frames
        # Output is the frame at position n_steps_input + k - 1 (i.e., k steps after the last input)
        output_idx = self.n_steps_input + k - 1

        # See WellDataset._construct_sample for the channels-first contract.
        variable_fields = data["variable_fields"].movedim(-1, 1)  # T x C x *spatial
        constant_fields = data["constant_fields"]
        if constant_fields.numel() > 0:
            constant_fields = constant_fields.movedim(-1, 0)  # C x *spatial
        ...
        sample = {
            "input_fields": variable_fields[: self.n_steps_input],          # Ti x C x *spatial
            "output_fields": variable_fields[output_idx : output_idx + 1],  # 1 x C x *spatial
            "constant_fields": constant_fields,                             # C x *spatial
            "input_scalars": data["variable_scalars"][: self.n_steps_input],  # Ti x C
            "output_scalars": data["variable_scalars"][output_idx : output_idx + 1],  # 1 x C
        }

        # Normalize lead-time: (k-1)/(max_k-1) maps [1, max_k] to [0, 1]
        if self.max_lead_time > 1:
            normalized_k = (k - 1) / (self.max_lead_time - 1)
        else:
            normalized_k = 0.0
        lead_time_tensor = torch.tensor([normalized_k], dtype=torch.float32)

        # Append lead-time to meta tensor
        if "constant_scalars" in data and data["constant_scalars"].numel() > 0:
            sample["meta"] = torch.cat([data["constant_scalars"], lead_time_tensor], dim=-1)
        else:
            sample["meta"] = lead_time_tensor

        if self.boundary_return_type is not None:
            sample["boundary_conditions"] = data["boundary_conditions"]

        if self.return_grid:
            sample["space_grid"] = data["space_grid"]
            sample["input_time_grid"] = data["time_grid"][: self.n_steps_input]
            sample["output_time_grid"] = data["time_grid"][output_idx : output_idx + 1]

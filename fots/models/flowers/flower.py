...
class SelfWarp(nn.Module):
    ...
    def forward(self, u, meta=None):
        ...
        base_grid_batch = self.base_grid.unsqueeze(0)
        grid = base_grid_batch + flow
...
class Flower(nn.Module):
    ...
    Args:
        ...
        boundary_condition_types: List of boundary conditions per dimension
...
class Flower2D(Flower):
    """Project-signature adapter for ``Flower`` on a 2D lat/lon grid.

    Bridges ``fots.train.build_model``'s ``(inp_shape, inp_chans, out_chans)``
    interface to Flower's ``(dim_in, dim_out, n_spatial_dims, spatial_resolution)``.
    Defaults boundary conditions to ``["ZEROS", "PERIODIC"]`` — zeros along
    latitude (poles aren't periodic; treat as out-of-bounds), periodic along
    longitude. Matches the convention used by the project's other models.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if boundary_condition_types is None:
            boundary_condition_types = ["ZEROS", "PERIODIC"]
        super().__init__(
            dim_in=inp_chans,
            dim_out=out_chans,
            n_spatial_dims=len(inp_shape),
            spatial_resolution=list(inp_shape),

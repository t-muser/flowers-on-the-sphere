...
This script:
1. Creates ``datagen/mitgcm/held_suarez/build/`` and runs ``genmake2`` with the
   ``-mods datagen/mitgcm/held_suarez/code/`` flag so our custom SIZE.h, packages.conf
   and apply_forcing.F override the defaults.
2. Runs ``make depend`` and ``make -j$(nproc)``.
3. Generates the static ``datagen/mitgcm/held_suarez/input/`` files that are symlinked
   into every run directory: ``bathyFile.bin`` and a build-info JSON.

The compiled executable lives at ``datagen/mitgcm/held_suarez/build/mitgcmuv``.
